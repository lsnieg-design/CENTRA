...
import { Building2, Palette, CalendarDays, SlidersHorizontal, Save, Plus, Trash2, CheckCircle2, RotateCcw, Image as ImageIcon, ShieldCheck, FileText, Settings2 } from 'lucide-react';
import { DEFAULT_APP_CONFIG, normalizeAppConfig, cacheAppConfig, applyBranding, PALETTES, MODULES, MODULE_CATALOG, FEATURE_LABELS, getRolePermissions, isModuleEnabled } from '../config';

const TABS = [
  { id: 'identity', label: 'Institución', icon: Building2 },
  { id: 'branding', label: 'Colores y logo', icon: Palette },
  { id: 'permissions', label: 'Permisos', icon: ShieldCheck },
  { id: 'labels', label: 'Nombres y documentos', icon: FileText },
  { id: 'lists', label: 'Listas', icon: SlidersHorizontal },
...
export function ConfiguracionView({ db, appId }) {
  ...
  const handleLogoUpload = (file) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        if (dataUrl.length > 300000) { alert('El logo quedó demasiado pesado. Probá una imagen más simple o con menos resolución.'); setLogoBusy(false); return; }
  ...
  const exportConfig = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `configuracion-institucional-${config.schoolYear || 'backup'}.json`; a.click(); URL.revokeObjectURL(url);
  };
  const importConfig = (file) => {
    if (!file) return;
    const reader = new FileReader();
    reader.onload = (event) => {
      try { const next = normalizeAppConfig(JSON.parse(event.target.result)); setConfig(next); setSelectedRole(next.roles?.[0] || ''); applyBranding(next); }
      catch { alert('El archivo de configuración no es válido.'); }
    };
  ...
  const save = async () => {
    ...
    try {
      await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'config', 'institution'), { ...normalized, updatedAt: serverTimestamp() }, { merge: true });
      cacheAppConfig(normalized); applyBranding(normalized);
      window.dispatchEvent(new CustomEvent('institution-config-updated', { detail: normalized }));
      setConfig(normalized); setSaved(true); setTimeout(() => setSaved(false), 3000);
    } catch { alert('No se pudo guardar la configuración. Revisá la conexión y los permisos.'); }
    finally { setSaving(false); }
  };
  const reset = () => {
    if (!confirm('¿Restaurar la configuración inicial? Esto reemplazará los cambios actuales en el formulario.')) return;
    const next = normalizeAppConfig(DEFAULT_APP_CONFIG); setConfig(next); setSelectedRole(next.roles?.[0] || ''); applyBranding(next);
  };
  const addHoliday = () => {
  ...
  const holidays = useMemo(() => (config.holidays || []).map(v => { const [date, name] = String(v).split('|'); return { raw: v, date, name: name || '' }; }), [config.holidays]);
  ...
  return (
    <div className="max-w-6xl mx-auto pb-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        <div><p className="text-xs font-black uppercase tracking-widest text-violet-600">Administración</p><h2 className="text-3xl font-black text-slate-900 tracking-tight">Configuración institucional</h2><p className="text-sm text-slate-500 mt-1">La institución se adapta desde acá. No hace falta tocar código.</p></div>
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={exportConfig} className="px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-600 font-bold text-xs">Exportar</button>
          <label className="px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-600 font-bold text-xs cursor-pointer">Importar<input type="file" accept="application/json" className="hidden" onChange={e => importConfig(e.target.files?.[0])}/></label>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-[215px_1fr] gap-5">
        <div className="bg-white border border-slate-200 rounded-2xl p-2 h-fit lg:sticky lg:top-4">
          {TABS.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => setTab(id)} className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-bold ${tab === id ? 'bg-violet-50 text-violet-700' : 'text-slate-500 hover:bg-slate-50'}`}><Icon size={18}/>{label}</button>)}
        </div>

        <div className="space-y-5">
          {tab === 'identity' && <>
            <section className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
              <div><h3 className="text-lg font-black">Datos de la institución</h3><p className="text-sm text-slate-500">Se usan en la app, comunicaciones y documentos.</p></div>
              <div className="grid md:grid-cols-2 gap-4">
                {[['institutionName','Nombre completo'],['institutionShortName','Nombre corto'],['portalTitle','Título del portal'],['appName','Nombre del sistema'],['email','Correo institucional'],['phone','Teléfono'],['address','Domicilio'],['city','Localidad'],['province','Provincia'],['website','Sitio web']].map(([key,label]) => <label key={key}><span className="text-xs font-black uppercase text-slate-500">{label}</span><input value={config[key] || ''} onChange={e=>update(key,e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 outline-none focus:ring-2 focus:ring-violet-200" /></label>)}
                ...
                <label><span className="text-xs font-black uppercase text-slate-500">País</span><input value={config.country || ''} onChange={e=>update('country',e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5" /></label>
              </div>
            </section>
          </>}
          ...
          {tab === 'branding' && <>
            <section className="bg-white rounded-2xl border border-slate-200 p-5 space-y-5">
              <div><h3 className="text-lg font-black">Identidad visual</h3><p className="text-sm text-slate-500">Elegí una paleta completa o personalizá los colores.</p></div>
          ...
          {tab === 'features' && <section className="bg-white rounded-2xl border border-slate-200 p-5 space-y-5">
            <div><h3 className="text-lg font-black">Módulos y funcionalidades</h3><p className="text-sm text-slate-500">Elegí qué partes del sistema estarán disponibles en esta instalación. Podés cambiarlo más adelante sin tocar el código.</p></div>
            <div className="rounded-2xl bg-violet-50 border border-violet-100 p-4 text-sm text-violet-900"><strong>Consejo:</strong> desactivá módulos que la institución no contrató o no necesita. Además de ocultarlos del menú, el sistema bloqueará su acceso directo.</div>
            <div className="grid md:grid-cols-2 gap-3">
              {MODULES.map(([id, label]) => {
                const meta = MODULE_CATALOG[id] || {};
                const enabled = isModuleEnabled(config, id);
                return <label key={id} className={`flex items-start gap-3 p-4 rounded-2xl border transition ${enabled ? 'bg-white border-slate-200' : 'bg-slate-50 border-slate-200 opacity-70'}`}>
                  <input type="checkbox" checked={enabled} disabled={!!meta.required} onChange={e=>updateModule(id,e.target.checked)} className="w-5 h-5 mt-0.5 accent-violet-600"/>
                  <span className="min-w-0"><span className="block text-sm font-black text-slate-800">{label}{meta.required ? ' · siempre disponible' : ''}</span><span className="block text-xs text-slate-500 mt-1">{meta.description || 'Funcionalidad del sistema.'}</span></span>
                </label>;
              })}
            </div>
            <div className="border-t border-slate-100 pt-4"><h4 className="text-sm font-black text-slate-800 mb-3">Compatibilidad con configuraciones anteriores</h4><div className="grid md:grid-cols-2 gap-3">{Object.entries(config.features || {}).map(([key,value])=><div key={key} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl"><span className="text-xs font-semibold text-slate-600">{FEATURE_LABELS[key] || key}</span><span className={`text-xs font-black ${value === false ? 'text-slate-400' : 'text-emerald-600'}`}>{value === false ? 'Desactivado' : 'Activo'}</span></div>)}</div><p className="text-[11px] text-slate-400 mt-2">Estos indicadores conservan compatibilidad con instalaciones antiguas. La selección principal se hace arriba.</p></div>
          </section>}
        </div>
      </div>
    </div>
